...
getLocals
  = filter ((== Local) . fst . snd)

getGlobals :: State -> State
getGlobals
  = filter ((== Global) . fst . snd)
...
assignArray (A a) (I i) (I v)
  = A ((i, v) : filter ((/= i) . fst) a) 

updateVar :: (Id, Value) -> State -> State
updateVar (id, v) s
  = case (lookup id s) of 
      Just result@(scope', v') -> (id, (scope', v)) : filter (/= (id, result)) s
      _                        -> (id, (Local, v)) : s
...
bindArgs ids
  = zip ids . zip (repeat Local)

evalArgs :: [Exp] -> [FunDef] -> State -> [Value]
evalArgs es fds s
  = map (\e -> eval e fds s) es
...
eval (Var id) _ s
  = getValue id s
eval (Cond p q r) fds s
  = case (eval p fds s) of
      I 1 -> eval q fds s
      _   -> eval r fds s
eval (OpApp op e e') fds s
  = applyOp op (eval e fds s) (eval e' fds s)
eval (FunApp f es) fds s
  = eval e fds (ba ++ s)
  where 
    (as, e) = lookUp f fds 
    vs      = evalArgs es fds s
    ba      = bindArgs as vs
...
executeStatement (AssignA id i e) fds _ s
  = updateVar (id, a') s
  where 
    a = getValue id s
    a' = assignArray a (eval i fds s) (eval e fds s)
executeStatement (If pe qb rb) fds pdfs s
  = case (eval pe fds s) of 
      I 1 -> executeBlock qb fds pdfs s
      _   -> executeBlock rb fds pdfs s
executeStatement st@(While pe b) fds pdfs s 
  = case (eval pe fds s) of 
      I 1 -> executeStatement st fds pdfs s' 
      _   -> s
  where 
    s' = executeBlock b fds pdfs s
executeStatement (Call vid pid es) fds pdfs s
  = case vid of 
      "" -> s''
      _  -> updateVar (vid, getValue "$res" s') s''
  where
    (ids, b) = lookUp pid pdfs
    vs       = evalArgs es fds s
    ba       = bindArgs ids vs 
    s'       = executeBlock b fds pdfs (ba ++ getGlobals s)
    s''      = getLocals s ++ getGlobals s'
...
executeBlock b fds pdfs s
  = foldl (\state st -> executeStatement st fds pdfs state) s b
...
translate' (FunApp fid es) m (id : ids)
  = ([Call id (lookUp fid m) es], Var id, ids)
translate' e@(Cond pe qe re) m ids
  | all null [qb, rb] = ([], e, ids)
  | otherwise         = ([If pe qb' rb'], Var id, ids'')
  where
    qb' = qb ++ [Assign id e']
    rb' = rb ++ [Assign id e'']
    (qb, e', ids')        = translate' qe m ids
    (rb, e'', id : ids'') = translate' re m ids'
translate' e@(OpApp op e1 e2) m ids
  = (b1 ++ b2, OpApp op e1' e2', ids2)
  where
    (b1, e1', ids1) = translate' e1 m ids
    (b2, e2', ids2) = translate' e2 m ids1
translate' e _ ids
  = ([], e, ids)
